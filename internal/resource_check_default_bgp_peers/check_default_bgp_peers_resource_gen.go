...
package resource_check_default_bgp_peers
...
func CheckDefaultBgpPeersResourceSchema(ctx context.Context) schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"kind": schema.StringAttribute{
				...
				Computed: true,
				Validators: []validator.String{
					stringvalidator.RegexMatches(regexp.MustCompile("^CheckDefaultBgpPeers$"), ""),
				},
				Default: stringdefault.StaticString("CheckDefaultBgpPeers"),
			},
			...
			"name": schema.StringAttribute{
				...
				MarkdownDescription: "name of the CheckDefaultBgpPeers",
			},
			"namespace": schema.StringAttribute{
				Optional:            true,
				Computed:            true,
			...
			"spec": schema.SingleNestedAttribute{
				Attributes: map[string]schema.Attribute{
					...
					"wait_for": schema.Int64Attribute{
						Optional: true,
					},
				},
				CustomType: SpecType{
			...
			"status": schema.SingleNestedAttribute{
				...
				MarkdownDescription: "CheckDefaultBgpPeersStatus defines the observed state of CheckDefaultBgpPeers",
			},
		},
	}
}

type CheckDefaultBgpPeersModel struct {
...
func (t SpecType) ValueFromObject(ctx context.Context, in basetypes.ObjectValue) (basetypes.ObjectValuable, diag.Diagnostics) {
	...
	waitForAttribute, ok := attributes["wait_for"]

	if !ok {
		diags.AddError(
			"Attribute Missing",
			`wait_for is missing from object`)

		return nil, diags
	}

	waitForVal, ok := waitForAttribute.(basetypes.Int64Value)

	if !ok {
		diags.AddError(
			"Attribute Wrong Type",
			fmt.Sprintf(`wait_for expected to be basetypes.Int64Value, was: %T`, waitForAttribute))
	}
...
type SpecValue struct {
	...
	WaitFor      basetypes.Int64Value `tfsdk:"wait_for"`
	state        attr.ValueState
}
...
func (v SpecValue) ToTerraformValue(ctx context.Context) (tftypes.Value, error) {
	...
	attrTypes["wait_for"] = basetypes.Int64Type{}.TerraformType(ctx)
	...
	case attr.ValueStateKnown:
		...
		val, err = v.WaitFor.ToTerraformValue(ctx)
...
func (v SpecValue) Equal(o attr.Value) bool {
	...
	if !v.WaitFor.Equal(other.WaitFor) {
		return false
	}
